...
Error EditorExportPlatformAndroid::run(const Ref<EditorExportPreset> &p_preset, int p_device, int p_debug_flags) {
	...
	const bool use_reverse = devices[p_device].api_level >= 19;
...
#define CLEANUP_AND_RETURN(m_err)                         \
	...
	if (use_remote) {
		if (use_reverse) {
			static const char *const msg = "--- Device API >= 19; debugging over USB ---";
			...
			static const char *const msg = "--- Device API < 19; debugging over Wi-Fi ---";
			EditorNode::get_singleton()->get_log()->add_message(msg, EditorLog::MSG_TYPE_EDITOR);
			print_line(String(msg).to_upper());
		}
	}
